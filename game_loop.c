 ...
 */
int game_loop_init(Game **game, Graphic_engine **gengine, char *file_name, Status log_status);
 ...
 * @return void (nothing)
 */
void game_loop_run(Game *game, Graphic_engine *gengine, Status log_status, char *log_name, char *output_name);
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2) {
    if (!strcmp(argv[2], "-l")) {
      if ((argc > 2) && (argc < 6)) {
        fprintf(stderr, "Use: %s %s %s <output_file> < <log_file>\n", argv[0], argv[1], argv[2]);
        return 1;
      }
      log_status = TRUE;
    }
